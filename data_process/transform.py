...
from PA_DDI.data_process.transform_utils.sampling import *
from PA_DDI.data_process.transform_utils.subgraph_extractors import *
import dgl
import re
import itertools
...
class SubgraphsTransform():
    ...
    def transform(self, data):
        ...
        if data.edge_index.shape == (0,):
            data.subgraphs_batch = torch.arange(data.num_nodes, dtype=torch.int64)
            data.subgraphs_nodes_mapper = torch.zeros(1)
            data.subgraphs_edges_mapper = torch.zeros(1)
            data.hop_indicator = torch.zeros(1)
            data.cut_RWPE = torch.zeros(data.num_nodes,16)
            data.ego_RWPE = torch.zeros(data.num_nodes,16)
            data.glo_RWPE = torch.zeros(data.num_nodes,20)
            data.subgraph_x_index = torch.arange(data.num_nodes, dtype=torch.int64)
